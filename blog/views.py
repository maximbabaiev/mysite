...
def post_share(request, post_id):
    """Извлекаем пост по индефикатору ID"""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, '673user666@gmail.com', [cd['to']])
            sent = True
...
def post_detail(request, year, month, day, post):
    post = get_object_or_404(Post,
                             ...
                             publish__day=day)
    comments = post.comments.filter(active=True)  # список активных коментариев к посту
...
def post_comment(request, post_id):
    ...
    if form.is_valid():
        comment = form.save(commit=False)  # создать обьект класса Comment не сохраняя его в базе
